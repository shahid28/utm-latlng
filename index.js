...
function getEllipsoid(name = "WGS 84") {
  ...
  if (typeof ellipsoids[name] !== "object") {
    throw new Error(
      `${name} is not valid, selection one of ${Object.keys(ellipsoids).join(
        ", "
      )}`
    );
  }
...
function toUtm(latitude, longitude, precision, ellipsoidName) {
  ...
  const N =
    a /
    Math.sqrt(
      1 - eccSquared * Math.sin(latitudeRadians) * Math.sin(latitudeRadians)
    );
  const T = Math.tan(latitudeRadians) * Math.tan(latitudeRadians);
  const C =
    eccPrimeSquared * Math.cos(latitudeRadians) * Math.cos(latitudeRadians);
  const A =
    Math.cos(latitudeRadians) * (longitudeRadians - longitudeOriginRadians);
